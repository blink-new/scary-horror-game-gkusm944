...
// Define a larger map for a longer game
const initialMap: MapCell[][] = [
  ...
  [
    { type: 'wall' }, { type: 'empty' }, { type: 'wall' }, { type: 'wall' }, { type: 'wall' }, { type: 'empty' }, { type: 'wall' }, { type: 'wall' }, { type: 'wall' }, { type: 'empty' }, { type: 'wall' },
  ],
  [
    { type: 'wall' }, { type: 'item', item: 'key' }, { type: 'wall' }, { type: 'empty' }, { type: 'wall' }, { type: 'empty' }, { type: 'wall' }, { type: 'empty' }, { type: 'wall' }, { type: 'empty' }, { type: 'wall' },
  ],
  [
    { type: 'wall' }, { type: 'empty' }, { type: 'wall' }, { type: 'empty' }, { type: 'wall' }, { type: 'empty' }, { type: 'wall' }, { type: 'empty' }, { type: 'wall' }, { type: 'empty' }, { type: 'wall' },
  ],
  [
    { type: 'wall' }, { type: 'empty' }, { type: 'wall' }, { type: 'empty' }, { type: 'wall' }, { type: 'empty' }, { type: 'wall' }, { type: 'empty' }, { type: 'wall' }, { type: 'empty' }, { type: 'wall' },
  ],
  ...
  [
    { type: 'wall' }, { type: 'empty' }, { type: 'empty' }, { type: 'empty' }, { type: 'empty' }, { type: 'empty' }, { type: 'empty' }, { type: 'empty' }, { type: 'empty' }, { type: 'exit' }, { type: 'wall' },
  ],
  [
    { type: 'wall' }, { type: 'wall' }, { type: 'wall' }, { type: 'wall' }, { type: 'wall' }, { type: 'wall' }, { type: 'wall' }, { type: 'wall' }, { type: 'wall' }, { type: 'wall' }, { type: 'wall' },
  ],
  ...
  useEffect(() => {
    ...
    const moveMonster = () => {
      ...
      for (const move of possibleMoves) {
        ...
        if (ny >= 0 && ny < map.length && nx >= 0 && nx < map[0].length && map[ny][nx].type !== 'wall') {
      ...
      if (validMoves.length > 0) {
        ...
        setMonster(prev => ({ x: prev.x + randomMove.dx, y: prev.y + randomMove.dy }))
      }
    }

    const interval = setInterval(moveMonster, 1000) // Monster moves every 1 second
    return () => clearInterval(interval)
  }, [scene, monster, map])

  // Check for monster collision after player moves or monster moves
  useEffect(() => {
    if (scene === 'game' && player.x === monster.x && player.y === monster.y) {
      setScene('death')
    }
